...
# Also build data with missing values filled in
weath_dat_filled <- portalr::weather(level = "monthly", fill = TRUE) %>%
    select(year, month, mintemp, maxtemp, meantemp, precipitation)
  
ndvi_dat_filled <- portalr::ndvi(level = "monthly", fill = TRUE) %>%
    mutate(year = lubridate::year(date), month = lubridate::month(date)) %>%
    left_join(weath_dat_filled, by = c("year", "month"))
